...
<style>
    ...
    .search-box { width: 350px; padding: 14px 16px; border-radius: 10px; border: 1px solid #ddd; background: #fff; outline: none; font-size: 14px; }
        ...
        <div class="hist-toolbar">
            <div class="search-group">
                <select name="search_type" id="search_type" class="filter-select" onchange="toggleInputs()">
                    <option value="bill_code" {{ $search_type === 'bill_code' ? 'selected' : '' }}>Tìm theo mã hóa đơn</option>
                    <option value="payment_status" {{ $search_type === 'payment_status' ? 'selected' : '' }}>Tìm theo trạng thái thanh toán</option>
                </select>

                <div id="q_input_group" style="display: {{ $search_type === 'bill_code' ? 'flex' : 'none' }}; gap: 10px; align-items: center;">
                    <input class="search-box" type="text" name="q" value="{{ $q }}" placeholder="Nhập mã hóa đơn..." autocomplete="off">
                </div>

                <div id="status_input_group" style="display: {{ $search_type === 'payment_status' ? 'flex' : 'none' }}; gap: 10px; align-items: center;">
            ...
            <div class="filters">
                ...
                @php
                    ...
                    $qs['sort'] = $toggleSort;
                @endphp
                <a class="filter-btn" href="{{ route('transaction_history', $qs) }}" style="text-decoration: none; color: #111;">
            ...
            <div class="card">
                <div class="card-row">
                    <div class="meta">
                        ...
                        <div><span style="color:#999;">Thời gian lập hóa đơn:</span> <b>{{ optional($bill->created_at)->format('d/m/Y H:i') }}</b></div>
                        <div>
