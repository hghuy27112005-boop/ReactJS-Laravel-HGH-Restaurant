...
<style>
    ...
    .spacer { flex: 1; }
    .footer-note { flex: 1; color: #777; font-size: 14px; font-style: italic; min-width: 200px; }
    ...
    .input-error { border-color: #ff4d4d !important; background-color: #fff2f2 !important; }
    
    /* Success Modal Style */
    .modal-success-content { background: #C0392B; color: white; text-align: center; padding: 40px; border-radius: 20px; width: 400px; margin: 10% auto; position: relative; box-shadow: 0 15px 30px rgba(192, 57, 43, 0.4); border: none; }
    ...
    .btn-success-close:hover { transform: scale(1.05); box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
    .close-success { position: absolute; top: 15px; right: 20px; color: white; font-size: 24px; cursor: pointer; opacity: 0.7; transition: 0.3s; }
    .close-success:hover { opacity: 1; transform: scale(1.2); }
...
<div class="cart-container">
    ...
    @php
        ...
        $deliveryItems = array_filter($items, function($item) { return ($item['order_type'] ?? '') === 'mang-ve'; });
        $dineInItems = array_filter($items, function($item) { return ($item['order_type'] ?? '') === 'dat-ban'; });
    ...
    <div class="cart-section">
        ...
        <table>
            <thead>
                <tr><th>STT</th><th>Tên Món</th><th>Số Lượng</th><th>Đơn Giá</th><th>Thành Tiền</th></tr>
            </thead>
            <tbody>
                @forelse($deliveryItems as $id => $item)
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td class="dish-info"><span class="name">{{ $item['name'] }}</span><span class="note">Ghi chú: {{ $item['note'] ?? 'Không có' }}</span></td>
                ...
                @if(count($deliveryItems) > 0)
                    <div class="info-group">
                        <div class="info-item">
                            ...
                            <div class="info-display-box" onclick="openModal('addressModal')">{{ session('user_address') ?? 'Nhấn để nhập...' }}</div>
                            <input type="hidden" id="address-mang-ve" value="{{ session('user_address') }}">
                        </div>
                        <button class="btn-action btn-edit" onclick="openModal('editQtyMangVeModal')" title="Chỉnh sửa số lượng"><i class="fas fa-edit"></i> Sửa</button>
                    </div>
                @endif
            ...
            @if(!session('last_confirmed_dat-ban'))
                ...
                @if(count($dineInItems) > 0)
                    <div class="info-group">
                        ...
                        <button class="btn-action btn-edit" onclick="openModal('editQtyDatBanModal')" title="Chỉnh sửa số lượng"><i class="fas fa-edit"></i> Sửa</button>
                    </div>
                @endif
...
<div id="addressModal" class="modal">
    <div class="modal-content">
        ...
        <div style="display:flex; justify-content: flex-end; gap: 10px; margin-top:15px;">
            <button class="btn-action" style="background:#eee;color:#333;" onclick="closeModal('addressModal')">Hủy</button>
    ...
    <div class="modal-content" style="width: 550px;">
        ...
        <div class="wizard-step active" id="step-1">
            ...
            <div style="display:flex; justify-content:center; gap:20px; margin:30px 0;">
                <label style="cursor:pointer; text-align:center;">
                    <input type="radio" name="total-tables" value="1" checked style="display:none;" onchange="updateTotalTables(1)">
                    <div class="table-count-opt" id="t-opt-1" style="width:60px; height:60px; border:2px solid #C0392B; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:24px; font-weight:bold; color:#C0392B; background:#fff5f5;">1</div>
                </label>
                <label style="cursor:pointer; text-align:center;">
                    <input type="radio" name="total-tables" value="2" style="display:none;" onchange="updateTotalTables(2)">
                    <div class="table-count-opt" id="t-opt-2" style="width:60px; height:60px; border:2px solid #eee; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:24px; font-weight:bold; color:#ccc;">2</div>
                </label>
                <label style="cursor:pointer; text-align:center;">
                    <input type="radio" name="total-tables" value="3" style="display:none;" onchange="updateTotalTables(3)">
                    <div class="table-count-opt" id="t-opt-3" style="width:60px; height:60px; border:2px solid #eee; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:24px; font-weight:bold; color:#ccc;">3</div>
                </label>
            </div>
        ...
        <div class="wizard-step" id="step-4">
            ...
            <div style="display:flex; justify-content:space-between; margin-top:20px;">
                ...
                <button class="btn-action" style="background:#C0392B;" id="btnNextStep4" onclick="goToStep(5)">Tiếp tục <i class="fas fa-arrow-right"></i></button>
            </div>
        </div>
...
<div id="editQtyMangVeModal" class="modal">
    <div class="modal-content" style="width: 600px;">
        ...
        <table>
            ...
            <tr>
                <td>{{ $item['name'] }}</td>
                <td><input type="number" class="modal-input qty-mangve" data-id="{{ $id }}" value="{{ $item['quantity'] }}" min="0" oninput="if(this.value < 0) this.value = 0;"></td>
            </tr>
            @endforeach
        </table>
        <div style="display:flex; justify-content: flex-end; gap: 10px; margin-top:15px;">
            <button class="btn-action" style="background:#eee;color:#333;" onclick="closeModal('editQtyMangVeModal')">Hủy</button>
...
<div id="successModal" class="modal" style="background: rgba(0,0,0,0.7);">
    <div class="modal-success-content" style="position: relative;">
        <span class="close-btn" style="position: absolute; top: 10px; right: 15px; font-size: 24px; cursor: pointer; color: white;" onclick="closeModal('successModal')">&times;</span>
